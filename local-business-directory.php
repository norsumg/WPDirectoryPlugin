...
/**
 * Change "Read More" to "View Business" in search results
 */
function lbd_change_read_more_text($more) {
    // Only change for business posts in search results
    if (is_search() && isset($_GET['post_type']) && $_GET['post_type'] === 'business') {
        return ' <a class="more-link" href="' . get_permalink() . '">View Business</a>';
    }
    
    return $more;
}
add_filter('excerpt_more', 'lbd_change_read_more_text', 10);

/**
 * Filter the read more text directly in the content for themes that use the_content_more_link filter
 */
function lbd_filter_content_more_link($link) {
    // Only change for business posts in search results
    if (is_search() && isset($_GET['post_type']) && $_GET['post_type'] === 'business') {
        return str_replace('Read more', 'View Business', $link);
    }
    
    return $link;
}
add_filter('the_content_more_link', 'lbd_filter_content_more_link', 10, 1);

/**
 * Add CSS to modify any remaining "Read More" links via CSS
 */
function lbd_modify_read_more_css() {
    ...
    ?>
    <style>
    body.search article.business a.more-link,
    body.search article.business a.read-more,
    body.search article.business a.read-more-button,
    body.search article.business .read-more a {
        display: inline-block;
        background: #0073aa;
    ...
    body.search article.business a.more-link:before,
    body.search article.business a.read-more:before,
    body.search article.business a.read-more-button:before,
    body.search article.business .read-more a:before {
        content: "View Business";
    }
    
    body.search article.business a.more-link span,
    body.search article.business a.read-more span,
    body.search article.business a.read-more-button span,
    body.search article.business .read-more a span {
        display: none;
    }
    </style>
    <?php
}
add_action('wp_head', 'lbd_modify_read_more_css', 1000); 